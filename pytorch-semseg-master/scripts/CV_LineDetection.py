...
def HueMask(image):
    ...
    for x in range(width):
        for y in range(height):
            ...
            if hist[index] >= 0.10 and index < max_val + 10 and index > max_val - 10:
                hsv[y,x] = [0,0,0]
            else:
                hsv[y,x] = [0,0,255]
...
def LineDetection(image, players_coord):
    ...
    _lines = cv2.HoughLinesP(mask,rho = 1,theta = 1*np.pi/180,threshold = 100,minLineLength = 80,maxLineGap = 10)
    lines = []
    ...
    SetLines(blank_image, grouped_lines, 1, offset_X, offset_Y)
...
def BestfitLines(lines):
    ...
    for x in range(4):
        if len(angles_4p[x]) >= 2:
            ...
            print('avg =', '%.3f'%avg_4p[x], 'std =', '%.3f'%std_4p[x], 'dif =', '%.3f'%dif_4p[x])
    print('========================')
    ...
    for x in range(len(lines)):
        ...
        if (abs(avg_4p[part] - angle) >= std_4p[part] and std_4p[part] > 5) or (abs(avg_4p[part] - angle) >= std_4p[part]*2 and dif_4p[part] > 10):
            ...
            print(part, 'bad', angle)
        else:
            fitted_lines.append(lines[x])
            print(part, 'good', angle)
...
def SetLines(image, lines, thickness, offset_x = 0, offset_y = 0):
    width = np.size(image, 1)
    height = np.size(image, 0)
...
for i in range(len(image_list)):
    ...
    image = LineDetection(image_list[i], all_players_coord[i])
